...
class MatRISModel(BaseModel):
    ...
    def __init__(
        ...
        use_precomputed_graphs: bool = False,  # 新增：是否使用预计算图
        **kwargs
    ) -> None:
        ...
        self.params = {
            ...
            "use_precomputed_graphs": use_precomputed_graphs,
        }
        ...
        # filter MatRIS param（type_map, sel, use_precomputed_graphs）
        matris_params = {k: v for k, v in self.params.items() 
                        if k not in ['type_map', 'sel', 'use_precomputed_graphs']}
    ...
    def set_systems(self, train_systems: list[str], valid_systems: list[str] = None):
        """
        设置 systems 路径列表（由 Trainer 调用）
        
        参数:
            train_systems: training system 目录路径列表
            valid_systems: validation system 目录路径列表（可选）
        
        ========== 改动说明 (new added in 2025-11-14) ==========
        目的：让模型知道training和validation的system路径
        调用时机：在training.py中，数据加载器创建之后
        使用方式：根据is_train参数选择使用train_systems还是valid_systems
        """
    ...
    def _load_precomputed_graphs(self, sid: int, fid: list[int], is_train: bool = True):
        ...
        参数:
            ...
            fid: frame ids列表
            is_train: 是否为 training（True）或 validation（False）
        
        返回:
            batch_graphs: RadiusGraph对象列表
        """
        import os
        # ========== 关键改动：根据 is_train 选择对应的 systems 列表 (new added in 2025-11-14) ==========
        # 目的：确保training和validation使用正确的system路径
        # - is_train=True  → 使用 train_systems[sid] → 加载 training 数据的图
        # - is_train=False → 使用 valid_systems[sid] → 加载 validation 数据的图
        # 如果不区分，validation时会错误地从train_systems加载，导致路径和索引错误
        if is_train:
            systems = self.train_systems  # ← Training时使用training的systems列表
        else:
            systems = self.valid_systems  # ← Validation时使用validation的systems列表
    ...
    def forward(
        ...
        # ========== 改动：使用预计算图时传递 is_train (new added in 2025-11-14) ==========
        # 目的：让_load_precomputed_graphs知道应该使用哪个systems列表
        if self.use_precomputed_graphs and sid is not None and fid is not None:
            # 加载预计算的图，传递is_train参数
